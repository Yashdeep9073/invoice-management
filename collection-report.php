...
try {
    $stmtFetch = $db->prepare("SELECT 
            ...
            COALESCE(SUM(CASE WHEN lt.transaction_type = 'INVOICE' THEN lt.debit_amount ELSE 0 END), 0) AS total_invoiced,
            COALESCE(SUM(CASE WHEN lt.transaction_type = 'PAYMENT' THEN lt.credit_amount ELSE 0 END), 0) AS total_collected,
            COALESCE(cw.balance, COALESCE(SUM(lt.debit_amount), 0) - COALESCE(SUM(lt.credit_amount), 0)) AS outstanding_amount,
            MAX(lt.transaction_date) AS last_transaction_date
        FROM customer c
        LEFT JOIN ledger_transactions lt ON c.customer_id = lt.customer_id
        LEFT JOIN customer_wallet cw ON c.customer_id = cw.customer_id
        GROUP BY 
            c.customer_id,
            c.customer_name,
            c.customer_phone,
                                ...
                                <tbody>
                                    ...
                                    foreach ($customers->fetch_all(MYSQLI_ASSOC) as $customer) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo (isset($localizationSettings["currency_symbol"]) ? $localizationSettings["currency_symbol"] : "$") . " " . number_format((float) $customer['total_invoiced'], 2) ?>
                                            </td>
                                            <td>
                                                <?php echo (isset($localizationSettings["currency_symbol"]) ? $localizationSettings["currency_symbol"] : "$") . " " . number_format((float) $customer['total_collected'], 2) ?>
                                            </td>
                                            <td>
                                                <?php echo (isset($localizationSettings["currency_symbol"]) ? $localizationSettings["currency_symbol"] : "$") . " " . number_format((float) $customer['outstanding_amount'], 2) ?>
                                            </td>
                                            <td>
                                                <?php echo !empty($customer['last_transaction_date']) ? date('d M Y h:i A', strtotime($customer['last_transaction_date'])) : '-' ?>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
